...
# def generate_new(ut_tokens, tokenizer, base_model_path, length_inf, length_sup, no_system=False):
#     template_name = find_template_name(base_model_path, no_system)
...
#     # input_text = system_format.format(content=system) + user_format.format(content=token)
#     user_text = user_format.
...
def generate_pure_ut_heuristic(ut_tokens, tokenizer, length_inf, length_sup):
    ...
    start_from_newline = ["llama-3", "qwen", "gemma"]
    ...
    assert len(text.strip()) == len(text)-1
    text = text.strip()
    return text
    # template = template_dict[template_name]
...
def generate_pure_ut(ut_tokens, tokenizer, length_inf, length_sup):
    model_name_or_path = tokenizer.model_name_or_path.lower()
